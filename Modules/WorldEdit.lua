name = "World Edit"
description =
"World Edit for bedrock. Type $help for an explanation (The wand isn't an axe, it's a sword). All commands use the \"$\" prefix."
...
event.listen("ChatMessageAdded", function(message, username, type, xuid)
    if string.sub(message, 1, 1) == "$" then --================================================================================================================FIND ARGUMENTS AND CMDNAME========================================================================================================================================================================================================================================================================================
            ...
            for xInSelectedRange = xpos1, xpos2, xstep do
                for yinSelectedRange = ypos1, ypos2, ystep do
                    for zinSelectedRange = zpos1, zpos2, zstep do
                        ...
                        tostring(xInSelectedRange) ..
                        " " .. tostring(yinSelectedRange) .. " " .. tostring(zinSelectedRange) .. " " .. BLOCKYTHING)
                    end
                end
            end
        end
                ...
                for yinSelectedRange = ypos1, ypos2, ystep do
                    for zinSelectedRange = zpos1, zpos2, zstep do
                        if (dimension.getBlock(xInSelectedRange, yinSelectedRange, zinSelectedRange).name == replacewhat) then
                            ...
                            client.execute("execute /setblock " ..
                            tostring(xInSelectedRange) ..
                            " " .. tostring(yinSelectedRange) .. " " .. tostring(zinSelectedRange) .. " " .. replacewith)
                        end
                    end
                end
            end
        ...
        if cmdname == "$line" then -- - - - - - - - - - - - - - - -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - - - - - - - - - - - - - - -- - - - - - - - - - - - - - - - - LINE -- - - - - - - - - - - - - - - -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  -- - - - - - - - - - - - - - - -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  -- - - - - - - - - - - - - - - -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            ...
            client.execute("execute /tp " ..
            xpos1 .. " " .. ypos1 .. " " .. zpos1 .. " facing " .. xpos2 .. " " .. ypos2 .. " " .. zpos2)
            xdif = xpos1 - xpos2
            ydif = ypos1 - ypos2
            zdif = zpos1 - zpos2
            ...
            for xInSelectedRange = xpos1, mirrorLine, 1 do
                for yinSelectedRange = ypos1, ypos2, 1 do
                    for zinSelectedRange = zpos1, zpos2, 1 do
                        ...
                        tostring(newx) ..
                        " " ..
                        tostring(yinSelectedRange) ..
                        " " ..
                        tostring(zinSelectedRange) ..
                        " " .. dimension.getBlock(xInSelectedRange, yinSelectedRange, zinSelectedRange).name)
                    end
                end
            end
        end
            ...
            if arg3 == "true" then
                if (arg1 == "x") then
                    for rot = -180, 180, 1 do
                        client.execute("execute /tp @s " ..
                        xpos1 .. " " .. ypos1 .. " " .. zpos1 .. " " .. rot .. " " .. "0")
                        client.execute("execute fill ~ ~ ~ ^ ^ ^" .. CIRCLERAD .. " " .. arg2)
                    end
                elseif (arg1 == "y") then
                    for rot = -180, 180, 1 do
                        client.execute("execute /tp @s" ..
                        xpos1 .. " " .. ypos1 .. " " .. zpos1 .. " " .. 90 .. " " .. rot)
                        arg2NEW = blockfiguer(arg2)
                        client.execute("execute fill ~ ~ ~ ^ ^ ^" .. CIRCLERAD .. " " .. arg2NEW)
                    end
                    for rot = -180, 180, 1 do
                        client.execute("execute /tp @s " .. xpos1 .. " " .. ypos1 .. " " .. zpos1 .. " " ..
                        " -90 " .. rot)
                        arg2NEW = blockfiguer(arg2)
                        client.execute("execute fill ~ ~ ~ ^ ^ ^" .. CIRCLERAD .. " " .. arg2NEW)
                    end
        ...
        if cmdname == "$wall" then -- - - - - - - - - - - - - - - -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  -- - - - - - - - - - - - - - - - - - WALL - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  -- - - - - - - - - - - - - - - -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  -- - - - - - - - - - - - - - - -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            ...
            for yness = math.min(ypos1, ypos2), math.max(ypos2, ypos1), 1 do
                client.execute("execute /tp " ..
                xpos1 .. " " .. yness .. " " .. zpos1 .. " facing " .. xpos2 .. " " .. yness .. " " .. zpos2)
                xdif = xpos1 - xpos2
                ydif = 0 -- it 2d idotss
                zdif = zpos1 - zpos2
                DISTANCE = math.sqrt(xdif * xdif + ydif * ydif + zdif * zdif)
                    ...
                    for zinSelectedRange = zpos1, zpos2, 1 do
                        ...
                        if (dimension.getBlock(xInSelectedRange, yinSelectedRange, zinSelectedRange).name == replacewhat) then
                            for howhighcount = 1, howhigh, 1 do
                                strtoexe = "execute /setblock " ..
                                tostring(xInSelectedRange) ..
                                " " ..
                                tostring(yinSelectedRange + howhighcount) ..
                                " " .. tostring(zinSelectedRange) .. " " .. replacewhat
                                --  print(strtoexe)
                ...
                for yinSelectedRange = ypos1, ypos2, ystep do
                    for zinSelectedRange = zpos1, zpos2, zstep do
                        ...
                        if (BLOCK.name == "air") then else
                            ...
                            for rot = -180, 180, 1 do
                                client.execute("execute /tp @s " ..
                                xpos1 .. " " .. yinSelectedRange .. " " .. zpos1 .. " " .. rot .. " " .. "0")
                    ...
                    for zinSelectedRange = zpos1, zpos1 + zdif222, 1 do
                        ...
                        if arg1 == "fasle" or nil then
                            if CURRENTBLOCK == "air" then
                                goto skip
                                print(
                                "§aSKipping air blocks when copying, not skipping will lag and maybe crash your game alot depending on size.§eTo not skip type $paste true")
                            end
                        else
                        ...
                        client.execute("execute /setblock " ..
                        xInSelectedRange .. " " .. yinSelectedRange .. " " .. zinSelectedRange .. " " .. CURRENTBLOCKDATA)
                        ::skip::
                        counter = counter + 2
                    end
                end
...
function render(dt)
    if xpos1 == nil or xpos2 == nil then else
        ...
        if areaOfSelection > 5000 then
            MuteChat = true
            client.execute(
            "execute title @s actionbar A lot of blocks are selected, this could cause unexpected behavior such as crashing, low framerate and freezes.")
        end
