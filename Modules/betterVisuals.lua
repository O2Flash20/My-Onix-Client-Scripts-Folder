...
thisContrast = nil
function render(dt)
    if not timeSinceUpdate then return end
    ...
    if midX and midY and trX and trY and tlX and tlY and brX and brY and blX and blY then
        ...
        gfx.tquad(
            (((tlX - midX) * 3 + midX) - w / 2) * 0.07 + w / 2, (((tlY - midY) * 3 + midY) - h / 2) * 0.07 + h / 2,
            0, 0,
            (((trX - midX) * 3 + midX) - w / 2) * 0.07 + w / 2, (((trY - midY) * 3 + midY) - h / 2) * 0.07 + h / 2,
            1, 0,
            (((brX - midX) * 3 + midX) - w / 2) * 0.07 + w / 2, (((brY - midY) * 3 + midY) - h / 2) * 0.07 + h / 2,
            1, 1,
            (((blX - midX) * 3 + midX) - w / 2) * 0.07 + w / 2, (((blY - midY) * 3 + midY) - h / 2) * 0.07 + h / 2,
            0, 1,
            "betterVisuals/squareFlare.png"
        )
...
function render3d()
    ...
    -- gfx.color(0, 0, 0, 100)
    gfx.tcolor(0, 0, 0, 100)
    for i = 1, #testFaces, 1 do
        -- gfx.quad(
        --     testFaces[i][1][1], testFaces[i][1][2], testFaces[i][1][3],
        --     testFaces[i][2][1], testFaces[i][2][2], testFaces[i][2][3],
        --     testFaces[i][3][1], testFaces[i][3][2], testFaces[i][3][3],
        --     testFaces[i][4][1], testFaces[i][4][2], testFaces[i][4][3],
        --     true
        -- )
    ...
    for x = 2, radius * 2 - 1, 1 do
        for y = 2, radius * 2 - 1, 1 do
            for z = 2, radius * 2 - 1, 1 do
                if blocks[x][y][z] then -- if the last thing is true, the face is facing in hte positive direction
        ...
        elseif math.floor(faces[i][2]) == faces[i][2] then --it's a y face
            for x = 1, resolution do
                for z = 1, resolution do
                    ...
                    if dimension.raycast(
                            faces[i][1] + xOffset, faces[i][2], faces[i][3] + zOffset,
